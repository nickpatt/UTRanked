...
import { Trophy, Briefcase, GraduationCap, ChevronRight, Equal, ArrowRight } from 'lucide-react';
...
function VotingPage() {
  ...
  const [leftStudent, setLeftStudent] = useState<Student>(students[0]);
  const [rightStudent, setRightStudent] = useState<Student>(students[1]);
  ...
  const handleEqual = () => {
    if (selectedId !== null) return; // Prevent multiple selections
    setSelectedId(-1); // Use -1 to indicate a tie
    setShowNext(true);
  };
  ...
  const handleNext = () => {
    setSelectedId(null);
    setShowNext(false);
    setLeftStudent(students[Math.floor(Math.random() * students.length)]);
    setRightStudent(students[Math.floor(Math.random() * students.length)]);
  };
  ...
  return (
    <div className="p-8 relative min-h-screen">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-8 text-white">Who has the better profile?</h1>
